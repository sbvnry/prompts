...
type StyleKey = keyof typeof stylePresets | "custom";

function buildPrompt(data: {
  skinTone: string; eyeColor: string; hair: string; hairColor: string; outfit: string; pose: string; props: string; expression: string; theme: string; background: string;
}) {
...
export default function Page() {
  ...
  const applyStyle = (selected: StyleKey) => {
    setStyle(selected);
    if (selected === "custom") return;

    const preset = stylePresets[selected];
  ...
  const Field = ({ label, value, setValue, list, disabled = false }: { label: string; value: string; setValue: (value: string) => void; list: string[]; disabled?: boolean }) => (
  ...
  const styleButtonClass = (key: StyleKey, activeClass: string, inactiveClass: string) =>
